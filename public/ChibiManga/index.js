...
var _Sources = (() => {
  ...
  // src/ChibiManga/ChibiManga.ts
  var ChibiManga_exports = {};
  __export(ChibiManga_exports, {
    ChibiManga: () => ChibiManga,
    ChibiMangaInfo: () => ChibiMangaInfo
  });
  ...
  // src/ChibiManga/ChibiMangaHelper.ts
  var CHIBI_MANGA_DOMAIN = "https://chibi-manga.com";
  var THAI_MONTHS = {
    "\u0E21\u0E01\u0E23\u0E32\u0E04\u0E21": 0,
    "\u0E01\u0E38\u0E21\u0E20\u0E32\u0E1E\u0E31\u0E19\u0E18\u0E4C": 1,
    "\u0E21\u0E35\u0E19\u0E32\u0E04\u0E21": 2,
  ...
  function parseThaiDate(dateStr) {
    ...
    if (trimmed.includes("\u0E17\u0E35\u0E48\u0E41\u0E25\u0E49\u0E27") || trimmed.includes("ago")) {
      ...
      if (trimmed.includes("\u0E27\u0E34\u0E19\u0E32\u0E17\u0E35") || trimmed.includes("second")) {
        return new Date(now.getTime() - amount * 1e3);
      } else if (trimmed.includes("\u0E19\u0E32\u0E17\u0E35") || trimmed.includes("minute") || trimmed.includes("min")) {
        return new Date(now.getTime() - amount * 60 * 1e3);
        ...
        return new Date(now.getTime() - amount * 365 * 24 * 60 * 60 * 1e3);
      }
      return now;
    }
  ...
  function parseChapterNumber(text3) {
    if (!text3) return 0;
    const match = text3.match(/(?:ตอนที่|chapter|ch\.?|ep\.?)\s*(\d+(?:\.\d+)?)/i) || text3.match(/(\d+(?:\.\d+)?)/);
  ...
  // src/ChibiManga/ChibiMangaParser.ts
  var ChibiMangaParser = class {
    /**
    ...
    static parseMangaTiles($2) {
      ...
      $2(".bs .bsx, .bsx").each((_, element) => {
        const linkTag = $2("a", element).first();
        const href = linkTag.attr("href") || "";
        const title = $2(".tt", element).text().trim() || linkTag.attr("title")?.trim();
        ...
        const subtitle = $2(".adds .epxs, .epxs, .adds .epx, .epx", element).first().text().trim() || void 0;
        tiles.push(
          App.createMangaTile({
            id,
            image: cleanUrl(image),
            title: App.createIconText({ text: title }),
            subtitleText: subtitle ? App.createIconText({ text: subtitle }) : void 0
          })
        );
      });
      if (tiles.length === 0) {
        $2(".utao").each((_, element) => {
          const linkTag = $2('.luf a.series, .series, a[href*="/manga/"]', element).first();
          const href = linkTag.attr("href") || "";
          ...
          if (!id || !title || seen.has(id)) return;
          seen.add(id);
          const imgTag = $2(".imgu img, img", element).first();
          const image = imgTag.attr("data-src") || imgTag.attr("src") || imgTag.attr("data-lazy-src") || "";
          const subtitle = $2(".luf ul.series li a", element).first().text().trim() || void 0;
          tiles.push(
            App.createMangaTile({
              id,
              image: cleanUrl(image),
    ...
    static parseMangaDetails($2, mangaId) {
      const title = $2("h1.entry-title").text().trim() || $2(".entry-title").text().trim() || $2(".info-right h1").text().trim();
      const imgTag = $2(".info-left .thumb img, .thumb img, img.wp-post-image").first();
      const image = imgTag.attr("data-src") || imgTag.attr("src") || $2('meta[property="og:image"]').attr("content") || "";
      const description = $2('.entry-content[itemprop="description"], .entry-content-single, .desc').first().text().trim();
      let status = "Ongoing";
      ...
      $2(".tsinfo .imptdt").each((_, el) => {
        ...
        if (text3.includes("\u0E1C\u0E39\u0E49\u0E40\u0E02\u0E35\u0E22\u0E19") || text3.includes("\u0E1C\u0E39\u0E49\u0E41\u0E15\u0E48\u0E07") || text3.includes("author") || text3.includes("artist")) {
          const val2 = $2(el).text().replace(/ผู้เขียน|ผู้แต่ง|author|artist|:/gi, "").trim();
          if (val2 && !author) {
            author = val2;
          }
        }
      ...
      const tags = [];
      $2(".mgen a, .seriestugenre a").each((_, el) => {
      ...
      const ratingText = $2(".rating-prc .num, .numscore").first().text().trim();
    ...
    static parseChapterList($2, mangaId) {
      ...
      $2("#chapterlist ul li, .bxcl ul li, .eplister ul li").each((_, element) => {
        ...
        const dateStr = $2(".chapterdate", element).text().trim();
    ...
    static hasNextPage($2) {
      return $2(".pagination .next, .hpage .r, a.next").length > 0;
    }
  };
  ...
  var ChibiManga = class extends import_types2.Source {
    constructor() {
      super(...arguments);
    ...
    async getMangaDetails(mangaId) {
      ...
      return ChibiMangaParser.parseMangaDetails($2, mangaId);
    }
    /**
    ...
    async getChapters(mangaId) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
      return ChibiMangaParser.parseChapterList($2, mangaId);
    }
    /**
    ...
    async getChapterDetails(mangaId, chapterId) {
      ...
      const $2 = load(html3);
      return ChibiMangaParser.parseChapterDetails(html3, mangaId, chapterId, $2);
    }
    /**
    ...
    async getHomePageSections(sectionCallback) {
      ...
      try {
        ...
        try {
          ...
          const latest$ = load(latestRes.data);
        ...
        try {
          const manhwaReq = App.createRequest({
            url: `${CHIBI_MANGA_DOMAIN}/genres/manhwa/`,
            method: "GET"
          });
          const manhwaRes = await this.requestManager.schedule(manhwaReq, 1);
          const manhwa$ = load(manhwaRes.data);
        ...
        try {
          const allReq = App.createRequest({
            url: `${CHIBI_MANGA_DOMAIN}/manga/?order=title`,
            method: "GET"
          });
          const allRes = await this.requestManager.schedule(allReq, 1);
          const all$ = load(allRes.data);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      switch (homepageSectionId) {
        case "popular":
          url = page > 1 ? `${CHIBI_MANGA_DOMAIN}/manga/page/${page}/?order=popular` : `${CHIBI_MANGA_DOMAIN}/manga/?order=popular`;
          break;
        case "latest":
          url = page > 1 ? `${CHIBI_MANGA_DOMAIN}/manga/page/${page}/?order=update` : `${CHIBI_MANGA_DOMAIN}/manga/?order=update`;
          break;
        case "manhwa":
          url = page > 1 ? `${CHIBI_MANGA_DOMAIN}/genres/manhwa/page/${page}/` : `${CHIBI_MANGA_DOMAIN}/genres/manhwa/`;
          break;
        case "all":
        default:
          url = page > 1 ? `${CHIBI_MANGA_DOMAIN}/manga/page/${page}/?order=title` : `${CHIBI_MANGA_DOMAIN}/manga/?order=title`;
          break;
      }
      const request = App.createRequest({
        url,
      ...
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
      const items = ChibiMangaParser.parseMangaTiles($2);
      return App.createPagedResults({
        results: items,
        metadata: ChibiMangaParser.hasNextPage($2) ? { page: page + 1 } : void 0
      });
    }
    /**
    ...
    async getSearchResults(query, metadata) {
      ...
      let url = "";
      if (query.title) {
        url = page > 1 ? `${CHIBI_MANGA_DOMAIN}/page/${page}/?s=${encodeURIComponent(query.title)}` : `${CHIBI_MANGA_DOMAIN}/?s=${encodeURIComponent(query.title)}`;
      } else {
        url = page > 1 ? `${CHIBI_MANGA_DOMAIN}/manga/page/${page}/?order=update` : `${CHIBI_MANGA_DOMAIN}/manga/?order=update`;
      }
      const request = App.createRequest({
  ...
  return __toCommonJS(ChibiManga_exports);
})();

this.Sources = _Sources;
if (typeof exports === 'object' && typeof module !== 'undefined') {
    module.exports.Sources = this.Sources;
    module.exports.ChibiManga = _Sources.ChibiManga;
    module.exports.ChibiMangaInfo = _Sources.ChibiMangaInfo;
}
if (typeof globalThis !== 'undefined') {
    globalThis.Sources = _Sources;
    globalThis.ChibiManga = _Sources.ChibiManga;
    globalThis.ChibiMangaInfo = _Sources.ChibiMangaInfo;
}
